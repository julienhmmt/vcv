...
- **Handlers**: `internal/handlers/` with separate registration functions per route group: `RegisterStaticRoutes`, `RegisterCertRoutes`, `RegisterAdminRoutes`, `RegisterI18nRoutes`. (The old HTMX `/ui/*` routes and `html/template` rendering have been removed.) Cert routes: `GET /api/certs` (list), `/api/certs/{id}/details`, `/api/certs/{id}/ca` (signing authority chain), `/api/certs/{id}/pem`. Plus `/api/health`, `/api/ready`, `/api/status`, `/api/version`, `/api/config` registered in `main.go`.
...
- **i18n**: `internal/i18n/i18n.go` is the source of truth — a `Messages` struct (~175 keys) with full maps for en/fr/de/it/es, served at `/api/i18n?lang=`. The frontend never hardcodes UI strings; it fetches these and looks them up by key via the shared `i18n` store (`t(key, fallback?, params?)`, `{{x}}`/`{x}` interpolation). Keep keys and frontend usage in sync — unreferenced keys are dead weight.
...
- **Components**: domain components in `src/lib/components/` — `CertDetailModal` / `CAModal` (passport-style detail + signing-authority viewer), `CertCard` (mobile ≤768px), `CertTypeSelect`, `CommandPalette` (Cmd/Ctrl-K), `Donut`, `ActiveFilters`, `StatusOverview`, `MountSelectorDialog`, `VaultStatusPill`, `ErrorBanner`, `TableSkeleton`. shadcn-svelte primitives live under `ui/`, admin-only under `admin/`.
