...
issue_cert_with_role() {
  ...
  if [ -n "${alt}" ]; then
    bao write "${mount}/issue/${role}" common_name="${cn}" alt_names="${alt}" ttl="${ttl}" >/dev/null 2>&1 || true
  else
    bao write "${mount}/issue/${role}" common_name="${cn}" ttl="${ttl}" >/dev/null 2>&1 || true
  fi
}
...
for mount in ${MOUNTS}; do
  ...
  bao write "${mount}/roles/vcv-machine" \
    allow_any_name=true \
    allow_bare_domains=true \
    allow_subdomains=true \
    enforce_hostnames=false \
  ...
  bao write "${mount}/roles/vcv-user" \
    allow_any_name=true \
    allow_bare_domains=true \
    allow_subdomains=true \
    enforce_hostnames=false \
  ...
  bao write "${mount}/roles/vcv-both" \
    allow_any_name=true \
    allow_bare_domains=true \
    allow_subdomains=true \
    enforce_hostnames=false \
  ...
  bao write "${mount}/roles/vcv-unknown" \
    allow_any_name=true \
    allow_bare_domains=true \
    allow_subdomains=true \
    enforce_hostnames=false \
